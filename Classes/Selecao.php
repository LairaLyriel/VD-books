...
class Selecao {
    ...
    function selecionarLivrosPorNota() {
        try {
            ...
            $stmt = $this->db->prepare(
                ...
                LEFT JOIN books_genders bg ON b.id_book = bg.books_id
    ...
    function selecionarItens() {
        $mangas = $this->selecionarMangasPorNota();
        $livros = $this->selecionarLivrosPorNota();
        $hqs = $this->selecionarHqPorNota();

        // Combinar todos os itens em um único array
        $itens = array_merge($mangas, $livros, $hqs);
    
        // Ordenar os itens pela média das notas
        usort($itens, function($a, $b) {
            // Usar media_nota se existir, caso contrário, considerar como 0
            $notaA = isset($a['manga_critic_note']) ? $a['manga_critic_note'] : (isset($a['critic_note']) ? $a['critic_note'] : 0);
            $notaB = isset($b['manga_critic_note']) ? $b['manga_critic_note'] : (isset($b['critic_note']) ? $b['critic_note'] : 0);
